...
export interface SEOAnalysis {
  ...
  images: {
    total: number;
    withoutAlt: number;
    issues: Array<{ severity: string; message: string; src?: string }>;
  };
...
class SeoAnalyzerService {
   ...
   */
  analyzeSEO(html: string, url: string, urlType: string = 'others'): SEOAnalysis {
   ...
   * Analyze title tag
   */
  private analyzeTitle($: cheerio.CheerioAPI): SEOAnalysis['title'] {
   ...
   */
  private analyzeMetaDescription($: cheerio.CheerioAPI): SEOAnalysis['metaDescription'] {
   ...
   */
  private analyzeOpenGraph($: cheerio.CheerioAPI): SEOAnalysis['openGraph'] {
   ...
   */
  private analyzeTwitterCard($: cheerio.CheerioAPI): SEOAnalysis['twitterCard'] {
   ...
   */
  private analyzeTechnicalSEO($: cheerio.CheerioAPI): SEOAnalysis['technical'] {
   ...
   */
  private analyzeHeadings($: cheerio.CheerioAPI): SEOAnalysis['headings'] {
    const h1Elements = $('h1');
   ...
   */
  private analyzeSchemas($: cheerio.CheerioAPI, urlType: string): SEOAnalysis['schemas'] {
   ...
   * Analyze images
   */
  private analyzeImages($: cheerio.CheerioAPI): SEOAnalysis['images'] {
    const images = $('img');
    const total = images.length;
    const withoutAlt: string[] = [];
    const issues: Array<{ severity: string; message: string; src?: string }> = [];
   ...
   * Analyze links
   */
  private analyzeLinks($: cheerio.CheerioAPI, pageUrl: string): SEOAnalysis['links'] {
    const links = $('a[href]');
    let internal = 0;
    let external = 0;
    let nofollow = 0;

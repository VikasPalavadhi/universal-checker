...
class RulesEngineEnhanced {
     ...
     */
    checkText(text, contentType = 'edm') {
        const issues = [];
        ...
        for (const rule of this.rules) {
            try {
                ...
                if (!appliesTo.includes(contentType))
                    continue;
                const regex = new RegExp(rule.pattern, 'gi');
                let match;
                while ((match = regex.exec(text)) !== null) {
                    const exceptions = rule.context_exceptions || [];
                    if (this.isException(text, match, exceptions))
                        continue;
                    const issue = {
                        rule_id: rule.id,
     ...
     */
    validateLinks(html, language = 'en') {
        const issues = [];
        if (!html)
            return issues;
        ...
        while ((match = linkPattern.exec(html)) !== null) {
            ...
            if (isMalformed)
                continue; // Skip further checks for malformed URLs
     ...
     */
    isSocialMediaLink(urlString) {
        try {
            const url = new URL(urlString);
            const hostname = url.hostname.toLowerCase().replace(/^www\./, '');
            return this.SOCIAL_MEDIA_DOMAINS.some(domain => hostname === domain || hostname.endsWith('.' + domain));
        }
        catch {
            return false;
        }
     ...
     */
    validateFontFamily(html, fileName) {
        const issues = [];
        if (!html)
            return issues;
    ...
    validateColors(html) {
        ...
        for (const pattern of colorPatterns) {
            ...
            while ((match = pattern.exec(html)) !== null) {
                ...
                const isAllowed = this.ALLOWED_COLORS.some(allowed => allowed.toLowerCase() === fullColor.toLowerCase());
                if (!isAllowed) {
                    issues.push({
    ...
    validateCustomVariables(text) {
        ...
        while ((match = validPattern.exec(text)) !== null) {
            ...
            if (!allowedVariables.includes(variableName)) {
                ...
                const correctVariable = allowedVariables.find(allowed => allowed.toLowerCase() === variableName.toLowerCase());
                if (correctVariable) {
    ...
    // HELPER METHODS
    // ============================================
    isException(text, match, exceptions) {
...
exports.rulesEngine = new RulesEngineEnhanced();

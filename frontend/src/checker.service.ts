...
// ⭐ NEW: Import DOCX and PDF parsers
...
class CheckerService {
   ...
   * ⭐ ENHANCED: Now supports HTML, Images, DOCX, and PDF with auto-detection
   * ⭐ ENHANCED: Detects multiple languages and tags issues by language
   */
  async checkFile(filePath: string, fileName: string): Promise<CheckResult> {
    ...
    try {
      ...
      // ⭐ ENHANCED: Extract text based on file type with auto-detection
      if (fileExt === 'html' || fileExt === 'htm') {
        ...
        // ⭐ NEW: DOCX files (Word documents)
        const docxResult = await this.extractTextFromDOCX(filePath);
        text = docxResult.text;
        html = docxResult.html; // Mammoth can extract HTML structure
        ocrUsed = false;
      }
      else if (fileExt === 'pdf') {
        // ⭐ NEW: PDF files
        ...
        throw new Error(`Unsupported file type: ${fileExt}. Supported: HTML, HTM, JPG, JPEG, PNG, DOCX, PDF`);
      }
  ...
  /**
   * ⭐ NEW: Extract text from DOCX files using Mammoth
   */
  private async extractTextFromDOCX(filePath: string): Promise<{ text: string; html: string }> {
    try {
  ...
  /**
   * ⭐ NEW: Extract text from PDF files using pdf-parse
   */
  ...
  /**
   * ⭐ NEW: Tag each issue with its language
   */
  private tagIssuesWithLanguage(issues: any[], sections: { english: string; arabic: string }, fullText: string): any[] {
    return issues.map(issue => {
      let language = 'both';  // Default to both if can't determine
      
      // Check if issue has original/found text to match against sections
      if (issue.original) {
        if (sections.english.includes(issue.original)) {
          language = 'english';
        } else if (sections.arabic.includes(issue.original)) {
          language = 'arabic';
        }
      } else if (issue.context) {
        // Use context to determine language
        if (sections.english.includes(issue.context)) {
          language = 'english';
        } else if (sections.arabic.includes(issue.context)) {
          language = 'arabic';
        }
      } else if (issue.found) {
